...
<head>
  ...
  <style>
    ...
    .card { max-width: 420px; padding: 1rem 1.25rem; border: 1px solid #ddd; border-radius: 12px; }
...
<body>
  <div class="card">
    ...
    <form method="get" action="/calc-ui">
      ...
      <label for="a">A (integer)</label>
      <input type="number" id="a" name="a" required value="{{ a if a is not none else '' }}">

      <label for="b">B (integer)</label>
      <input type="number" id="b" name="b" required value="{{ b if b is not none else '' }}">
    ...
    {% else %}
      <p class="muted">Tip: choose an operation and enter A & B.</p>
    {% endif %}
...
def create_app():
    ...
    def calc():
        ...
        try:
            if op == "add":
                ...
                return jsonify(error="op must be one of add|sub|mul|div"), 400
    ...
    def calc_ui():
        ...
        # first load: show page without result
        ...
        try:
            if op == "add":
                ...
                error = "op must be one of add|sub|mul|div"
        except ZeroDivisionError as e:
            error = str(e)
